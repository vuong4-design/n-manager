<div align="center">
  <h1>n-manager</h1>
  <p><strong>Local account pool, dashboard, and protocol proxy for AI</strong></p>
...
- `Reverse Proxy` for the full Notion AI web UI: `/ai/<account>` aliases hand off to isolated `<account>.localhost/ai` browser origins
...
- Use a separate researcher path without a hard-coded numeric trial limit

### Dashboard

- Embedded React dashboard at `/dashboard/`
- Password login with session cookies
- Pool view with **server-side search, status filters, sort, and pagination** (`q`, `status`, `page`, `page_size`)
- Per-account actions: open proxy, copy token, manually disable/enable, **delete account file + pool entry**
- Multi-select across pages, including one-click selection of every account matching the current search and status filter
- Checks, enable/disable, delete, and cleanup run as background jobs with 10 workers by default, live progress, refresh-safe status, and failed-item retry
...
- Add Account accepts one token per line and validates up to five tokens concurrently; the import dialog can keep all accounts or only accounts with a configured default-Agent personal-instructions binding
...
- Open multiple Notion AI accounts in one browser session; each `/ai/<account>` entry opens a separate `<account>.localhost:<port>/ai` origin
